...
package org.infinispan.spring.ai.vectorstore;
...
public class InfinispanFilterExpressionConverterTest {
	...
	@Test
	public void shouldEscapeSingleQuoteInValue() {
		String filter = this.converter
			.convertExpression(new Filter.Expression(EQ, new Filter.Key("name"), new Filter.Value("O'Brien")));
		assertThat(filter).isEqualTo("m0.name='name' and m0.value='O''Brien'");
	}

	@Test
	public void shouldEscapeSingleQuoteInKey() {
		String filter = this.converter
			.convertExpression(new Filter.Expression(EQ, new Filter.Key("o'clock"), new Filter.Value("noon")));
		assertThat(filter).isEqualTo("m0.name='o''clock' and m0.value='noon'");
	}

	@Test
	public void shouldEscapeBackslashInValue() {
		String filter = this.converter
			.convertExpression(new Filter.Expression(EQ, new Filter.Key("path"), new Filter.Value("C:\\Users\\test")));
		assertThat(filter).isEqualTo("m0.name='path' and m0.value='C:\\\\Users\\\\test'");
	}

	@Test
	public void shouldEscapeSingleQuoteInInFilter() {
		String filter = this.converter.convertExpression(
				new Filter.Expression(IN, new Filter.Key("name"), new Filter.Value(List.of("O'Brien", "D'Angelo"))));
		assertThat(filter).isEqualTo("m0.name='name' and m0.value IN ('O''Brien', 'D''Angelo')");
	}
