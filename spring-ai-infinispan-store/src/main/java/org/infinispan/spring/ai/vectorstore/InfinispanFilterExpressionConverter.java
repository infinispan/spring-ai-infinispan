...
class InfinispanFilterExpressionConverter extends AbstractFilterExpressionConverter {
	...
	private String computeValue(String operator, Object value) {
		Object normalizedValue = normalizeDateString(value);
		String m = "m" + this.i + ".";
		if (normalizedValue instanceof Integer || normalizedValue instanceof Long) {
			Long longValue = getLongValue(normalizedValue);
			return m + "value_int" + operator + longValue;
		}
		else if (normalizedValue instanceof Float || normalizedValue instanceof Double) {
			Double doubleValue = getDoubleValue(normalizedValue);
			return m + "value_float" + operator + doubleValue;
		}
		else if (normalizedValue instanceof Date || normalizedValue instanceof Instant) {
			return m + "value_date" + operator + getDate(normalizedValue);
		}
		else if (normalizedValue instanceof Boolean bool) {
			return m + "value_bool" + operator + bool.booleanValue();
		}
		else {
			return m + "value" + operator + escape(String.valueOf(normalizedValue));
		}
	}
